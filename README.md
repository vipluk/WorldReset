...
* **📈 Leaderboards & Records Database:** Built-in `records.yml` database tracks attempts, completions, PBs, deaths and average times. Tracks a **Top 10 Highscore Leaderboard** with player names, record times, dates, and world seeds.
* **📊 Vanilla Scoreboard Integration:** Plug-and-play synchronization with **39 vanilla scoreboard objectives**! Track and display speedrun metrics (attempts, completions, total deaths, current run deaths, average times, personal bests, server records, timer live seconds/minutes, seed, world name, active players, difficulty, active goal, death reset, etc.) using built-in Minecraft commands, completely out of the box! Enable/disable it instantly using `/wr scoreboard`.
...
| `/wr autoreset after <number>` | Configures death limit to trigger autoreset (Shared Lives). | `worldreset.autoreset.config` |
